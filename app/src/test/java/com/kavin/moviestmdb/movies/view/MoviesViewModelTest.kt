...
import com.kavin.moviestmdb.CoroutineTestingRule
...
import com.kavin.moviestmdb.movies.data.MovieResponse
import com.kavin.moviestmdb.movies.domain.MovieEntity
import com.kavin.moviestmdb.utils.commons.TestDispatcher
...
class MoviesViewModelTest {
    ...
    fun test_GetTrailers_success() {
        ...
        runBlockingTest {
            ...
            moviesViewModel.apply {
                ...
                uiState.observeForever {
                    val actual = ((it as MoreDetailViewState.TrailersFetchedSuccess).trailers)
    ...
    fun test_GetTrailers_failure() {
        runBlockingTest {
            ...
            moviesViewModel.apply {
                fetchMovieTrailers(1)
                uiState.observeForever { assert(it is MoreDetailViewState.TrailersFetchedError) }
                verify(moviesRepo, times(1)).fetchMovieTrailers(any())
            }
        }
    }
    ...
    fun test_GetMovieLikeState() {
        ...
        runBlockingTest {
            ...
            moviesViewModel.apply {
                getLikeState(1)
                uiState.observeForever {
                    val actual = ((it as MoreDetailViewState.LikeState).isLiked)
                    assert(expected == actual)
                }
